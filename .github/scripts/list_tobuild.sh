...
if [ ! -s tobuild.txt ]; then
      ...
      grep -Pzo "(?s)\s*\"\N*\":\s*\[\s*\]" packages.json | awk -F'"' '{print $2}' | grep -v '^$' | xargs -i bash -c 'touch lists/{} && if ! [ -s "lists/failed/{}" ]; then if ! [ -s "lists/{}" ]; then if grep -q "tar.gz" "lists/{}"; then echo "readytobuild" >> lists/{}; fi; fi; fi'
